    ...
    public partial class Form1 : Form
    {
        ...
        private void button27_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                System.Drawing.Rectangle bound = this.Bounds;    //Screen.AllScreens[0].Bounds;
                Bitmap captureBitmap = new Bitmap(bound.Width, bound.Height, PixelFormat.Format32bppArgb);
                //Tạo đối tượng Rectangle sẽ chụp lại màn hình
                Graphics captureGraphics = Graphics.FromImage(captureBitmap);
                //Copying Image from The Screen
                captureGraphics.CopyFromScreen(bound.Left, bound.Top, 0, 0, bound.Size);
                SaveFileDialog sfd = new SaveFileDialog();
                sfd.Title = "Save As JPEG";
                sfd.FileName = "ScreenShot.jpg";
                sfd.Filter = "Image Files(*.BMP;*.JPG;*.GIF;*.PNG)|*.BMP;*.JPG;*.GIF;*.PNG|All files (*.*)|*.*";
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    captureBitmap.Save(sfd.FileName, ImageFormat.Jpeg);
                    MessageBox.Show("Screen Captured","Capture Successfully !");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            GUI.EditAccount ec = new GUI.EditAccount();
            ec.SetAcc(user, userpass);
            ec.Visible = true;
        }
        ...
        private void đặtLạiỨngDụngToolStripMenuItem_Click(object sender, EventArgs e)
        {
            color = new Color();
        ...
        private void xóaBảngNhânViênToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeleteNV nv = new NotificationGUI.DeleteNV();
            nv.Visible = true;
        }

        private void xóaBảngSubteamToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeleteSubteam subteam = new NotificationGUI.DeleteSubteam();
            subteam.Visible = true;
        }

        private void xóaBảngMônHọcToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeleteMH mh = new NotificationGUI.DeleteMH();
            mh.Visible = true;
        }

        private void xóaBảngCôngViệcToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeleteCV cv = new NotificationGUI.DeleteCV();
            cv.Visible = true;
        }

        private void xóaBảngĐồÁnToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeleteDoAn da = new NotificationGUI.DeleteDoAn();
            da.Visible = true;
        }

        private void xóaBảngPhânCôngToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            NotificationGUI.DeletePhanCong pc = new NotificationGUI.DeletePhanCong();
            pc.Visible = true;
        }

        private void xóaBảngChiTiếtMônHọcToolStripMenuItem1_Click(object sender, EventArgs e)
        {
        ...
        private void ImportExcel1()
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                // Lấy danh sách sheet trong workbook  
                ...
                for (int i = 1; i <= 10; i++)
                {
                    ...
                    tb.Items.Add(new ListViewItem(strArray));
                }
            }
        }
        ...
        private void cmbx4_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbx4.SelectedIndex == 0)
            {
                ExportExcel4();
            }
            else
