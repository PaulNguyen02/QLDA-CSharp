...
namespace WindowsFormsApp1.DAO
{
    ...
    {      
        ...
        public List<DTO.CongViec> Search1(String searchid)
        {
            ...
            {
                conn.Open();
                SqlCommand command = new SqlCommand("select * from Congviec, MonHoc, DoAn  where MonHoc.MAMH = @mamh and DOAN.MAMH=MONHOC.MAMH and CONGVIEC.MADA=DOAN.MADA", conn);
        ...
        public int Count(String searchid)
        {
            ...
            {
                conn.Open();
                SqlCommand command = new SqlCommand("select count(MACV) from Congviec, MonHoc, DoAn where MonHoc.MAMH = @mamh and DOAN.MAMH=MONHOC.MAMH and CONGVIEC.MADA=DOAN.MADA", conn);
                command.Parameters.Add(new SqlParameter("@mamh", searchid));
                //using (SqlDataReader reader = command.ExecuteReader())
                //{
        ...
        public int CountFinished(String searchid)
        {
            ...
            {
                conn.Open();
                SqlCommand command = new SqlCommand("select count(MACV) from Congviec, MonHoc, DoAn where Stawork=1 and MonHoc.MAMH = @mamh and DOAN.MAMH=MONHOC.MAMH and CONGVIEC.MADA=DOAN.MADA", conn);
                command.Parameters.Add(new SqlParameter("@mamh", searchid));
                //using (SqlDataReader reader = command.ExecuteReader())
                //{
                    count = (Int32)command.ExecuteScalar();
        ...
        public string SearchSubject(string search)
        {
            try
            {
                conn.Open();
                SqlCommand cmd;
                SqlDataReader reader;
                cmd = new SqlCommand("select * from Congviec, MonHoc, DoAn where MonHoc.MAMH = @mamh and DOAN.MAMH=MONHOC.MAMH and CONGVIEC.MADA=DOAN.MADA", conn);
                cmd.Parameters.Add(new SqlParameter("@mamh", search));
                reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    subjectname = Convert.ToString(reader.GetValue(8));
                }
            }
            catch (SqlException sqlex)
            {
